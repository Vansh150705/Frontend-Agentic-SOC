...
export const ThreatsPage = () => {
  ...
  function handleExport() {
    exportToPDF({
      ...
      headers: ["ID", "Date", "User", "Role", "Threat Type", "Severity"],
      rows:    filtered.map(t => [t.id, t.date, t.user, t.role, t.type, t.severity]),
    });
  }
        ...
        <div className="flex items-center gap-1">
          {["All", "Critical", "HIGH", "MEDIUM", "LOW"].map((s) => (
            <button key={s} onClick={() => setSeverityFilter(s)}
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${severityFilter === s ? "bg-gray-900 text-white" : "text-gray-500 hover:bg-gray-100 hover:text-gray-800"}`}>
              {s}
            </button>
          ))}
        </div>
        ...
        {!loading && !error && (
          <Table headers={["ID", "Date", "User", "Role", "Threat Type", "Severity"]}>
            {filtered.map((t, index) => (
              <tr key={index} className={`hover:bg-gray-50 transition-colors ${t.severity === "Critical" ? "border-l-2 border-red-400" : ""}`}>
                ...
                <td className="px-4 py-3"><span className="bg-slate-100 text-slate-500 px-1.5 py-0.5 rounded text-[10px] font-mono">{t.role}</span></td>
